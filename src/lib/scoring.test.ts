...
describe("PATTERNS", () => {
  ...
  describe("爛位風/正位風 (wind meld(s) vs. seat wind)", () => {
    ...
    it("爛位風 stacks: 2 wrong-wind melds score 4, one row each", () => {
...
describe("PATTERNS: TaiPattern.tiles (tap-to-expand tile breakdowns)", () => {
  it("將眼: points at just the pair, e.g. 55t out of 123456789m111222z55t - a single row", () => {
    const c = ctx();
  ...
  it("三元牌: each dragon meld is its own row, stacking", () => {
    const c = ctx();
  ...
  it("明清龍/暗清龍: a ResolvedMeld[][]-backed stacking pattern - only the specific 789m copy the claimed tile completed shows under -open, the other under -hidden, each its own single row", () => {
    ...
    expect(patternTiles(result, "pure-straight-open", c)).toEqual([expectedRow]);
    expect(patternTiles(result, "pure-straight-hidden", c)).toEqual([expectedRow]);
  });

  it("單色步步高: 3 overlapping ascending-sequence instances render as 3 separate rows - 123234345456567m11z", () => {
    // The user's own worked example: 5 runs shifted by 1 rank each
    // (123/234/345/456/567m) form 3 overlapping combineSegments-style
    // instances - [123,234,345], [234,345,456], [345,456,567] - each of
    // which should be its own row, not all flattened together.
    const c = ctx();
    const result = scoreHand("123m234m345m456m567m11z", c);
    expect(tai(result, "same-suit-consecutive-hidden")).toBe(90); // 3 instances x 30
    expect(patternTiles(result, "same-suit-consecutive-hidden", c)).toEqual([
      [[{ suit: "m", rank: 1 }, { suit: "m", rank: 2 }, { suit: "m", rank: 3 }], [{ suit: "m", rank: 2 }, { suit: "m", rank: 3 }, { suit: "m", rank: 4 }], [{ suit: "m", rank: 3 }, { suit: "m", rank: 4 }, { suit: "m", rank: 5 }]],
      [[{ suit: "m", rank: 2 }, { suit: "m", rank: 3 }, { suit: "m", rank: 4 }], [{ suit: "m", rank: 3 }, { suit: "m", rank: 4 }, { suit: "m", rank: 5 }], [{ suit: "m", rank: 4 }, { suit: "m", rank: 5 }, { suit: "m", rank: 6 }]],
      [[{ suit: "m", rank: 3 }, { suit: "m", rank: 4 }, { suit: "m", rank: 5 }], [{ suit: "m", rank: 4 }, { suit: "m", rank: 5 }, { suit: "m", rank: 6 }], [{ suit: "m", rank: 5 }, { suit: "m", rank: 6 }, { suit: "m", rank: 7 }]],
    ]);
  });

  it("老少上: a count-only stacking pattern (no ResolvedMeld[][] helper) - recomputes the contributing 123m/789m melds directly, as a single row", () => {
    const c = ctx();
