...
(meld/pair) within that instance — the Scoring tab's Patterns list uses this to let the user tap a
pattern's name and see just those tiles expand underneath it, one line per instance (e.g. tapping
...
| 不搭三相逢 (Same 3 ranks across all 3 suits) | 20 | The 3 unrelated ranks chosen are the *same* 3 ranks in all of m/t/b — e.g. `159m+159t+159b` — **and** the hand's own pair is an honor tile. | — | **Additive bonus** — stacks with either 十六不搭 or 十六不搭(十六飛), not an alternative to them. Per the user, this only fires when the pair is honor - `decomposeSixteenUnrelated` doesn't actually favor honors when picking which kind gets doubled (whichever kind happens to hold 2 copies becomes the pair), so a numbered pair can otherwise satisfy the exact same rank-matching condition (rank *sets* per suit are unaffected by which kind within them is doubled) - this is a real, explicitly-checked condition, not a structural consequence of the shape. Mutually exclusive with 不搭雜龍 below by construction (the ranks' union can't be size 3 and size 9 at once), so no explicit exclude was needed between them. |
| 不搭雜龍 (Ranks span 1-9 across all 3 suits) | 20 | The 3 suits' unrelated ranks collectively cover 1-9 with no rank repeated across suits — e.g. `147m+258b+369t` — **and** the hand's own pair is an honor tile. | — | **Additive bonus**, same framing (and same honor-pair requirement) as 不搭三相逢. Each suit already contributes exactly 3 distinct ranks in a valid 十六不搭 hand, so the ranks' union hitting all 9 possible values is only possible with zero overlap between suits. |
...
| 明絕 (Won on a tile already declared 3 times) | 5 | The 食胡 tile's kind already sits **exactly 3 times** across this hand's own DECLARED (exposed) melds — most often a single declared triplet of that kind, but just as easily spread across 3 separate declared runs each holding one copy, or any other combination of declared melds that happens to sum to exactly 3. Does **not** require the concealed hand's own wait to be single — the wait can genuinely still be open to other, un-exhausted kinds too (see 絕絕 below for when the *whole* wait is exhausted like that); this only cares about the one kind that actually completed the hand. **Also fires when the user manually declares it** (`GameContext.manualVisibleTripleWin`) - the auto-check has no visibility into the discard pile or other players' melds, so a button lets the user assert it anyway when they know it independently; scored as `autoCheck OR (manualFlag AND not blocked)`, so no double-counting when both agree. **The manual half is blocked** when the 食胡 tile's kind is *also* sitting elsewhere in this hand's own concealed tiles (`isWinningTileHeldConcealedElsewhere`) — e.g. the winning tile completed a concealed pair, and the player was already holding the pair's other tile. Unlike the discard pile or other players' melds, this tool *can* see the player's own concealed hand directly, so a manual claim of "this was the last copy anywhere" that's directly contradicted by data already in hand gets blocked regardless of what the user says they saw elsewhere. This check deliberately does **not** apply to the auto-detect half above — the game's fixed 4-copies-per-kind limit already makes that collision structurally impossible whenever `declaredCopiesOfTile === 3` (3 declared plus the winning tile's own concealed occurrence already accounts for all 4 copies, leaving no room for a second one). The UI shares a single three-state button with 絕絕 below (off / 明絕 / 絕絕) rather than two separate toggles, since 絕絕 is structurally always a stronger finding than 明絕 when it applies; the button can't be cycled below whatever floor the auto-checks already prove (locked entirely on 絕絕 once the exhausted-wait auto-check alone is true), nor above the concealed-duplicate ceiling when that block applies. **Also applies to all 3 special hands** (十三么/十六不搭/嚦咕嚦咕) via the manual override — their auto-detect half is structurally always false there (always zero declared melds by construction), but the concealed-duplicate block above still meaningfully limits the manual half: for 嚦咕嚦咕 specifically, it's *always* blocked, since a valid pairs hand holds every tile kind at least twice concealed by construction, so any winning-tile choice always collides. | — | "明" (visible) specifically: only exposed melds count toward the 3 — a declared/exposed kong of that kind would push the total to 4 (falls outside this pattern, no separate kind check needed to exclude it), and a concealed (self-drawn) kong doesn't count at all, since it isn't visible to anyone else at the table, same as every other concealed meld. Stacks with 獨獨 when the wait does happen to also be single (they measure different things: 獨獨 is about the wait's shape, this is about visible scarcity on the completing tile). **Caveat, surfaced in the app too as a tooltip on this pattern's row**: this only ever looks at *this hand's own* declared melds — it has no visibility into the discard pile or any other player's declared melds, so it can't actually confirm the 食胡 tile was the last live copy anywhere in the game, only that this hand's own exposed melds already show 3 of them. |
| 絕絕 (Multi-way wait narrowed to one visible copy) | 10 | An extension of 明絕. The concealed hand's genuine pre-completion wait (the same `getWaits` computation 獨獨 uses) has **2 or more** distinct tile kinds — a nominally multi-way wait — but this hand's own DECLARED (exposed) melds already account for every copy of those waiting kinds except a **single one anywhere**: summing `4 - (copies visible in this hand's own declared melds)` across every waiting kind comes out to exactly 1. E.g. declared `333m666m567m`, concealed `45m` waiting on `3m`/`6m`: `3m` has 3 declared copies (1 left anywhere), `6m` has all 4 (3 from `666m`'s triplet plus 1 from `567m`'s own `6m` — none left) — only one tile, `3m`, could actually still complete this nominally two-sided wait. An exposed kong works the same way (e.g. declared `333m` plus an exposed `6666m` kong also accounts for all 4 of `6m`'s copies), but a **concealed** kong doesn't count, same "only what's visible" reasoning as everywhere else here. **Also fires when manually declared** (`GameContext.manualVisibleExhaustedMultiWait`), same `autoCheck OR (manualFlag AND not blocked)` shape and the same concealed-duplicate block, via the same shared button as 明絕 above (its top step). **One more gate on top, specific to 絕絕**: the manual half also requires `isGenuineMultiWait` — this hand's own concealed pre-completion wait must actually have 2+ distinct tile kinds (same `getWaits` check the auto-detect half already uses), independent of anything declared. Unlike the "how many copies are already visibly declared" half (which genuinely needs the discard pile / other players' melds the manual override exists to cover), whether the wait was multi-way to begin with is entirely a fact about this hand's own concealed tiles — fully visible to this tool — so a manual 絕絕 claim on a hand whose own wait was only ever a genuine single wait is blocked, same reasoning as the concealed-duplicate gate, just checking a different fact. This second gate doesn't affect 明絕 at all (明絕 doesn't care about wait count), so on a single-wait hand the shared button can still reach 明絕 manually, just not 絕絕. **Consequence for the 3 special hands**: 明絕's manual override does apply to all 3 (see its own row above), but 絕絕's is effectively *always* blocked for all 3 — 十三么/十六不搭 always reduce to a genuine single wait right before completion (the one open orphan/unrelated slot, or 十三么's own tacked-on meld, is the only thing left; everything else is already locked in as singles), and 嚦咕嚦咕 is blocked by the *other* gate instead (every kind is held at least twice concealed by construction). | 明絕 | Excludes 明絕 since it's the same underlying "visibly exhausted" idea, just a stronger, more specific finding when it applies — the two CAN genuinely co-occur (明絕 doesn't require the wait to be single, so its own raw score can still be positive on the one collided kind even when the overall wait is multi-way), so this exclusion is load-bearing, not just defensive: confirmed against the worked example above, where 明絕's own raw score is 5 (it would fire on its own) but is suppressed once 絕絕 fires. Naturally disjoint from 獨獨 (獨獨 requires exactly 1 wait candidate, this requires 2+). **Caveat, surfaced in the app too as a tooltip on this pattern's row, same as 明絕**: only checks this hand's own declared melds — no visibility into the discard pile or any other player's declared melds, so it can't confirm these are truly the last copies anywhere in the game. |
| 自摸 (Self-drawn win) | 1 | `selfDraw` is true. | — | Excluded by 門清自摸. Also applies to all 3 special hands (十三么/十六不搭/嚦咕嚦咕) - but 門清自摸 deliberately never fires there (see its own row), so this is always the one that applies when self-drawn. **UI-only wiring, not part of `selfDraw` itself**: the 自摸 button also lights up (and `GameContext.selfDraw` is forced true) whenever 花摸, 槓摸, 河底撈魚, 海底撈月/海底撈月(一筒), or 天胡 is declared - all mean the winning tile was drawn/dealt to the player themselves (per this house rule, including 河底撈魚), which is inherently a self-draw. Tapping 自摸 off while held on this way cascades all of those back to their off state too, rather than leaving them declared with no visible way to un-toggle them. Conversely, 自摸 (in any of these forms) is mutually exclusive with 搶槓, 雙響/三響, 地胡, and 食叮 below - all mean the win was claimed off another player, the opposite of a self-draw - so activating 自摸 clears all four, and activating any of them clears 自摸 (and, transitively, every field listed above along with it). 人胡 is deliberately left unlinked either way - see its own row. |
| 門清自摸 (Self-drawn win while 門前清) | 3 | Self-drawn **and** 門前清 (see the Foundation section) both hold. | 自摸 | Upgrade of 自摸 - excludes it, but stacks with 門前清 itself (they measure different things: one about melds, one about how the tile arrived). Deliberately excluded from all 3 special hands' declared-only pattern battery, per the user - those hands are always concealed by construction, so this would otherwise always fire instead of plain 自摸 whenever self-drawn, the same way 十三么 itself already treats plain 門前清 as "trivially true, not meaningful to list" rather than something worth stacking a bonus on top of. |
| 叮 (Riichi) | 5 | `riichi === "riichi"` - a plain declared state (`GameContext.riichi`, a 4-way cycle: none/叮/天叮/地叮), toggled directly by the user in the UI. Nothing about the hand's own shape determines this, unlike almost every other pattern in this table. | — | Excluded by 門清叮. Also applies to all 3 special hands, same as 自摸 above - 門清叮 deliberately never fires there. |
| 門清叮 (Riichi while 門前清) | 10 | Riichi declared (`riichi === "riichi"`) **and** 門前清 both hold. | 叮 | Same shape as 自摸/門清自摸 - upgrade of 叮, excludes it, but stacks with 門前清 itself. Deliberately excluded from all 3 special hands, same reasoning as 門清自摸 above. |
| 天叮 (Heavenly Riichi) | 60 | `riichi === "heavenly-riichi"` - the dealer (莊) declares 叮 immediately after discarding their very first tile. | 叮, 門清叮 | Mutually exclusive with 叮/地叮 by construction too (`riichi` is a single state), but the exclusion is listed explicitly per the user, same reasoning as 絕絕/明絕 elsewhere in this doc. Also applies to all 3 special hands. |
| 地叮 (Earthly Riichi) | 50 | `riichi === "earthly-riichi"` - a non-dealer declares 叮 after discarding their very first tile following their first draw. | 叮, 門清叮 | Mutually exclusive with 叮/天叮 by construction too, same reasoning as 天叮 above. Also applies to all 3 special hands. |
| 一發 | 5 | `instantWin` is true **and** `riichi` isn't `"none"` - the hand completed within the immediate round after declaring 叮 (any of its 3 states). | — | Stacks additively on top of whichever 叮/門清叮/天叮/地叮 tai already applies - not a further upgrade of those, just its own flat bonus. **UI-only wiring**: the button is always visible but disabled while 叮 is `"none"` (unlike 食叮 below, which stays independently togglable regardless of 叮's state) - resets back to false the moment 叮 cycles back to "none" so it can't linger as a stale true value behind a disabled button with no way to un-toggle it. Also applies to all 3 special hands. |
| 食叮 | 5 | `eatRiichi` is true - independent of `riichi`'s own state entirely, per the user's own house rule (this scores even with `riichi === "none"`, unlike 一發 which requires it). | — | Independent of 一發 - both can be declared at once, each contributing their own flat 5 tai. Also applies to all 3 special hands. **UI-only wiring**: unlike 一發 (whose button is disabled while `riichi === "none"`), 食叮's button is always enabled, matching that it always scores regardless of 叮's state. Joins the 搶槓/雙響/三響/地胡 "claimed win" group (see 自摸's own row): eating into the completed hand off a discard is a claimed win, so activating 食叮 forces 自摸 off (and 花摸/槓摸/河底撈魚/海底撈月/天胡 with it), and activating 自摸 forces 食叮 back off. |
| 四子內 (Won within 4 discards) | 60 | `earlyWin === "four"` - a plain declared state (`GameContext.earlyWin`, a 4-way cycle: none/四子內/七子內/十子內, independent of 叮), toggled by the user in the UI. Means the hand was won while the total discard count (excluding the tile that completed the hand) was still at or under the stated number. | — | Mutually exclusive with 七子內/十子內 by construction (`earlyWin` is a single state). Also applies to all 3 special hands. **UI-only wiring**: also mutually exclusive with 河底撈魚/海底撈月/海底撈月(一筒) below - the wall can't be both nearly full (early in the hand) and exhausted (the very last tile) at once, so activating either cycle clears the other. |
| 七子內 (Won within 7 discards) | 30 | Same shape as 四子內, `earlyWin === "seven"`. | — | Mutually exclusive with 四子內/十子內 by construction. Also applies to all 3 special hands. Same 河底撈魚/海底撈月/海底撈月(一筒) mutual-exclusion wiring as 四子內. |
| 十子內 (Won within 10 discards) | 15 | Same shape as 四子內, `earlyWin === "ten"`. | — | Mutually exclusive with 四子內/七子內 by construction. Also applies to all 3 special hands. Same 河底撈魚/海底撈月/海底撈月(一筒) mutual-exclusion wiring as 四子內. |
| 雙響 | 5 | `multiWin === "double"` - a plain declared state (`GameContext.multiWin`, a 3-way cycle: none/雙響/三響), toggled by the user in the UI, independent of everything else in this table. Means multiple players won off the same discard. | — | Mutually exclusive with 三響 by construction. Also applies to all 3 special hands. **UI-only wiring** (see 自摸's own row): activating 雙響/三響 forces 自摸 off (and 花摸/槓摸/海底撈月/天胡 back to their off state with it), and activating 自摸 forces 雙響/三響 back to none - a discard-claimed win and a self-draw can't both be true. |
| 三響 | 10 | Same shape as 雙響, `multiWin === "triple"`. | — | Mutually exclusive with 雙響 by construction. Also applies to all 3 special hands. Same 自摸 mutual-exclusion UI wiring as 雙響. |
| 天胡 | 160 | `heavenlyWin === "heaven"` - a plain declared state (`GameContext.heavenlyWin`, a 4-way cycle: none/天胡/地胡/人胡), toggled by the user in the UI, independent of everything else in this table. | — | Mutually exclusive with 地胡/人胡 by construction. Also applies to all 3 special hands. **UI-only wiring** (see 自摸's own row): the dealer's initial deal being already complete is inherently a self-draw, so activating 天胡 forces 自摸 on (deactivating 搶槓/雙響/三響/地胡 with it); activating 自摸 forces 天胡 back to none if it was the reason 自摸 was on. All 3 of 天胡/地胡/人胡 are also mutually exclusive with 河底撈魚/海底撈月/海底撈月(一筒) below - won on essentially the very first tile vs. the very last one, opposite ends of the hand - so activating any of the 3 clears `lastTileWin`, and activating any lastTileWin state clears `heavenlyWin`. |
| 地胡 | 120 | Same shape as 天胡, `heavenlyWin === "earth"`. | — | Mutually exclusive with 天胡/人胡 by construction. Also applies to all 3 special hands. **UI-only wiring**: per this house rule, 地胡 is winning off the very first discard - claimed, not self-drawn - so it joins 搶槓/雙響/三響's group: activating 地胡 forces 自摸 off (and 花摸/槓摸/河底撈魚/海底撈月/天胡 with it), and activating 自摸 forces 地胡 back to none. 人胡 is deliberately left unlinked to 自摸 either way. Same 河底撈魚/海底撈月/海底撈月(一筒) mutual-exclusion wiring as 天胡. |
| 人胡 | 80 | Same shape as 天胡, `heavenlyWin === "man"`. | — | Mutually exclusive with 天胡/地胡 by construction. Also applies to all 3 special hands. No 自摸 UI wiring (unlike 天胡/地胡 above) - deliberately left unasserted either way. Same 河底撈魚/海底撈月/海底撈月(一筒) mutual-exclusion wiring as 天胡/地胡, despite not being linked to 自摸. |
| 河底撈魚 | 5 | `lastTileWin === "river-bottom"` - a plain declared state (`GameContext.lastTileWin`, a 4-way cycle: none/河底撈魚/海底撈月/海底撈月一筒), toggled by the user in the UI, independent of everything else in this table. Won by claiming another player's very last discard of the game. | — | Mutually exclusive with 海底撈月/海底撈月(一筒) by construction. Also applies to all 3 special hands. **UI-only wiring**: per this house rule, activating 河底撈魚 also forces 自摸 on (same as 海底撈月/海底撈月(一筒) below), and 自摸-off clears it back to none. Also mutually exclusive with 四子內/七子內/十子內 (see their own row) and with 天胡/地胡/人胡 (see their own rows). |
| 海底撈月 | 10 | `lastTileWin === "sea-bottom"`. Won by self-drawing the very last tile in the wall. | — | Mutually exclusive with 河底撈魚 by construction (`lastTileWin` is a single state). Excluded by 海底撈月(一筒) below when that upgrade fires. Also applies to all 3 special hands. **UI-only wiring** (see 自摸's own row): self-drawing the last wall tile is inherently a self-draw, so activating 海底撈月 forces 自摸 on (deactivating 搶槓/雙響/三響/地胡 with it); activating 自摸 forces 海底撈月 back to none if it was the reason 自摸 was on. Also mutually exclusive with 四子內/七子內/十子內 and 天胡/地胡/人胡, same as 河底撈魚. |
| 海底撈月(一筒) | 20 | `lastTileWin === "sea-bottom"` **and** `selfDraw` **and** the winning tile is exactly 1 Tong (`1t`). | 海底撈月 | **Not its own declared state** - `LastTileWinState` only has 3 values (none/river-bottom/sea-bottom); this is an automatic upgrade of plain 海底撈月, the same "auto-upgrade excludes the base" shape as 門清自摸/門清叮, fired purely from the winning tile happening to be 1t once 海底撈月 and a self-draw both hold - there's no separate button/state for the user to declare it directly. `selfDraw` is checked explicitly in the pattern's own condition even though the UI already always forces it true alongside `sea-bottom` - scoring.ts doesn't rely on any particular UI's wiring to stay correct standalone. Also applies to all 3 special hands. Same 四子內/七子內/十子內 and 天胡/地胡/人胡 UI wiring as 海底撈月 (inherited from cycling `lastTileWin` to "sea-bottom" in the first place). |
| 花摸 | 2 each | `flowerDraw * 2` - `GameContext.flowerDraw` is a declared count, independent of everything else in this table. Means a bonus-tile replacement draw completed/advanced the hand, that many times. | — | Purely declared - the app has no concept of draw order, so unlike a real 花摸 ruling (which genuinely depends on exactly when each bonus tile was revealed relative to the winning tile) this just trusts the user's count. **UI-only wiring**: the cycle button's cap isn't a fixed ceiling - it's however many bonus tiles are actually in the hand right now (`bonusTiles.length`), disabled entirely with no bonus tiles declared. If a bonus tile is later removed, an already-declared count above the new (lower) cap is clamped down automatically rather than left stale. Also applies to all 3 special hands. |
| 槓摸 | `FIVE_POWER_TAI_TABLE[n]` | `GameContext.kongDraw`, a declared count. Tai follows the literal table `[0, 5, 25, 125, 250, 250]` indexed by the count - **not** a clean `5^n` formula (5⁴ and 5⁵ both cap at 250, per the user's own worked numbers, not the `min(5^n, 240)` they also mentioned - the explicit list is treated as ground truth over the mismatched formula description). | — | Same "purely declared, trust the user" reasoning as 花摸 - a kong-replacement draw completed/advanced the hand, that many times. **UI-only wiring**: same live-cap idea as 花摸, but off the number of kongs actually declared (`kongCount`, concealed or exposed - either counts) rather than bonus tiles; disabled with no kongs declared, and an already-declared count clamps down automatically if a kong is later removed. Also applies to all 3 special hands. |
| 搶槓 | `FIVE_POWER_TAI_TABLE[n]` | Same shape as 槓摸 (`GameContext.robKong`, same table), fully independent of it - robbing the kong (winning by claiming a tile another player just added to upgrade an existing pon into a kong). | — | Also applies to all 3 special hands. **UI-only wiring** (see 自摸's own row): activating 搶槓 forces 自摸 off (and 花摸/槓摸/海底撈月/天胡 with it), and activating 自摸 forces 搶槓 back to 0 - robbing a kong is won off another player, the opposite of a self-draw. |
| 莊 (連莊 - consecutive dealer wins) | `2n - 1` | `GameContext.dealerStreak`, a declared count (0 = not declared, 1 = just 莊 with no streak yet - 1 tai, n ≥ 2 = a 莊連(n-1) streak - 2(n-1)+1 = 2n-1 tai). Unlike 花摸/槓摸/搶槓, this is a plain linear formula, not a lookup table - uncapped above 0, since there's no fixed physical-tile ceiling behind a dealer's win streak the way there is for bonus tiles or kongs. | — | Fully independent of everything else in this table - a dealer can extend their streak by either self-drawing or claiming a discard, so this doesn't join either the self-draw or claimed-win UI-wiring group. Also applies to all 3 special hands. **UI-only wiring**: tapping the button toggles between inactive (shown as plain "莊", 0 tai) and just-declared (also shown as plain "莊", but now active, 1 tai); +/- buttons appear next to it once active, stepping it into 莊連1 (3 tai), 莊連2 (5 tai), and so on. Tapping the button again while active resets it back to inactive - there's no separate "Reset only" affordance for this one field, the button itself doubles as it. |
...
| Pattern | Tai | Criteria | Excludes | Notes |
|---|---|---|---|---|
| 二連刻 (2 consecutive triplets/kongs) | 5 each | 2 triplets/kongs at consecutive ranks in one suit (e.g. `222m+333m`, triplet/kong mixed freely). No 明/暗 split. | — | **Stacks per adjacent pair** — 3 consecutive triplets (`222+333+444`) counts as 2 instances. Only discounts the pairs actually subsumed by 小/大三連刻 (whichever specific 3-rank window(s) those found) - a genuinely separate adjacent pair using up the hand's *other* melds still counts, e.g. `111m222m333m555t666t` claims 大三連刻 for the m-suit window but still separately scores 二連刻 for `555t+666t`. |
| 小三連刻 (3 consecutive ranks, pair at one end) | 15 each | 3 consecutive ranks in one suit where the hand's pair sits at one end and the other 2 are triplets/kongs — e.g. `22m+333m+444m`. | — | Structurally different from 大三連刻 (needs the pair), so not mutually exclusive with it - confirmed. Its own claimed melds subtract from 二連刻's count (see that row) rather than a blanket exclude. **Stacks** across the pair's up-to-3 sliding windows (pair at the low end, middle, or high end) — e.g. `111m222m444m555m` around a `33m` pair scores 3 instances (all of low/middle/high qualify with triplets flanking on both sides), same reasoning as 大三連刻/清龍. |
| 大三連刻 (3 consecutive triplets/kongs) | 30 each | Same 3-consecutive-rank shape, but all 3 are full triplets/kongs — pair not involved. | — | Its own claimed melds subtract from 二連刻's count (see that row) rather than a blanket exclude. **Stacks** across overlapping windows — `111m222m333m444m555m` scores 3 instances (`[1,2,3]`, `[2,3,4]`, `[3,4,5]`), same sliding-window reasoning as 二連刻/清龍. |
...
| Pattern | Tai | Criteria | Excludes | Notes |
|---|---|---|---|---|
| 相逢 (Same run, different suits) | 3 each | 2 runs with the same 3 ranks but in different suits — e.g. `234m+234t`. No 明/暗 split. | — | **Stacks per instance** - a run held once in one suit pairs with *every* copy the other suit holds (shared, not consumed by forming one pairing), same non-exclusive-pairing principle as [清龍/雜龍/步步高's own stacking](#duplicate-instance-counting-清龍雜龍老少上老少碰步步高二步高): `123m+123m+123t` forms **2** instances (each `123m` paired with the one shared `123t`), not 1. Only discounts the instances actually subsumed by whichever of 明/暗三/四/五相逢 fires (whatever specific rank that tier claims) - a genuinely separate same-run pairing at a *different* rank still counts, e.g. `123m789m123t789t123b` claims 暗三相逢 for the rank-1 triple but still separately scores 相逢 for `789m+789t`. |
| 雙姊妹 (2 distinct 相逢 instances) | 5 | A flat bonus on top of 相逢, once per hand, when 2 *distinct* 相逢 instances exist — sharing no meld between them. E.g. `123m+123b` and `678t+678b`. | — | **Additive, not exclusive** — stacks with 相逢 itself (it's a bonus on top, not an alternative reading). A shape like `123m+123m+123t` forms 2 相逢 instances (see 相逢's own row) but both share the same `123t`, so they aren't 2 *distinct* instances and this doesn't qualify. |
...
| 明三相逢 (Same run in all 3 suits, open) | 10 | All 3 suits have a run at the same rank — e.g. `567m+567t+567b` — instance is 明. | — | Its own claimed melds subtract from 相逢's count (see that row) rather than a blanket exclude of the whole pattern. |
| 暗三相逢 (same, concealed) | 20 | Same shape, instance is 暗. | — | Same scoped-subtraction relationship with 相逢 as 明三相逢. |
| 明四相逢 (4 runs across all 3 suits, one suit doubled, open) | 40 | The 3-suit group plus one extra run in whichever suit has it twice — e.g. `456m456m456t456b` — instance is 明. | 明三相逢, 暗三相逢 | Same scoped-subtraction relationship with 相逢 as 明三相逢 (not a blanket exclude) - still fully excludes 明/暗三相逢 themselves, since whichever rank this tier finds always also satisfies the weaker 3-suit threshold at that same rank. |
| 暗四相逢 (same, concealed) | 80 | Same shape, instance is 暗. | 明三相逢, 暗三相逢 | Same as 明四相逢. |
| 明五相逢 (5 runs across all 3 suits, open) | 80 | All 3 suits present with 5 runs total spread across them however they fall (2+2+1, 3+1+1, etc.) — e.g. `234234m234234t234b` or `234234234m234t234b` — instance is 明. | 明/暗三相逢, 明/暗四相逢 | Same scoped-subtraction relationship with 相逢 as 明三相逢. |
| 暗五相逢 (same, concealed) | 160 | Same shape, instance is 暗. | 明/暗三相逢, 明/暗四相逢 | Same as 明五相逢. |
| 兩兄弟 (Same triplet/kong rank, different suits) | 5 each | 2 triplets/kongs at the same rank but different suits — e.g. `555t+555b`. No 明/暗 split. | — | **Stacks per instance**. Subject to the same 4-copies-per-tile limit as everywhere else - unlike 相逢's runs, a triplet/kong rank can never repeat within one suit, so the cross-product-vs-index-pairing distinction that mattered for 相逢 never actually comes up here. Only discounts the instances actually subsumed by whichever of 小/大三兄弟 fires - a genuinely separate same-rank pairing at a *different* rank still counts, e.g. `111m111t111b777m777b` claims 大三兄弟 for the rank-1 triple but still separately scores 兩兄弟 for `777m+777b`. |
| 小三兄弟 (Same rank across 3 suits, pair at one) | 20 | One rank held as the pair (in its own suit), with the other 2 suits each holding a triplet/kong at that *same* rank — e.g. `33t+333m+3333b`. | — | Structurally different from 大三兄弟 (needs the pair), so not mutually exclusive with it — same precedent as 小/大三連刻. Its own claimed melds subtract from 兩兄弟's count (see that row) rather than a blanket exclude. |
| 大三兄弟 (Same rank across all 3 suits, all triplets/kongs) | 40 | All 3 suits hold a triplet/kong at the same rank — e.g. `555m+555t+555b`. Pair not involved. | — | Its own claimed melds subtract from 兩兄弟's count (see that row) rather than a blanket exclude. |
...
- **Added kong vs. called kong** aren't distinguished — both parse/score as a generic "exposed
  kong." Originally noted as mattering only for 搶槓 (robbing the kong) - but 搶槓 ended up
  implemented as a purely-declared count (`GameContext.robKong`, same shape as 槓摸) rather than
  something derived from kong type at all, so this gap no longer blocks anything; it's just still
  true that the notation itself can't tell the two apart.
