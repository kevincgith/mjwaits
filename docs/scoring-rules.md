...
| 莊 (連莊 - consecutive dealer wins) | `2n + 1` | `GameContext.dealerStreak`, a declared count (0 = not declared). Unlike 花摸/槓摸/搶槓, this is a plain linear formula, not a lookup table - uncapped above 0, since there's no fixed physical-tile ceiling behind a dealer's win streak the way there is for bonus tiles or kongs. | — | Fully independent of everything else in this table - a dealer can extend their streak by either self-drawing or claiming a discard, so this doesn't join either the self-draw or claimed-win UI-wiring group. Also applies to all 3 special hands. **UI-only wiring**: tapping the button toggles between inactive (0) and 莊x1 (3 tai); +/- buttons appear next to it once active to step the count further, and tapping the button again while active resets it back to inactive - there's no separate "Reset only" affordance for this one field, the button itself doubles as it. |
