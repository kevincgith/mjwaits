...
| 自摸 (Self-drawn win) | 1 | `selfDraw` is true. | — | Excluded by 門清自摸. Also applies to all 3 special hands (十三么/十六不搭/嚦咕嚦咕) - though in practice 門清自摸 always wins there instead, since all 3 are always concealed by construction; see the Known gaps note below. |
| 門清自摸 (Self-drawn win while 門前清) | 3 | Self-drawn **and** 門前清 (see the Foundation section) both hold. | 自摸 | Upgrade of 自摸 - excludes it, but stacks with 門前清 itself (they measure different things: one about melds, one about how the tile arrived). Also applies to all 3 special hands, where it always wins over plain 自摸 (see the Known gaps note below). |
| 叮 (Riichi) | 5 | `riichi === "riichi"` - a plain declared state (`GameContext.riichi`, a 4-way cycle: none/叮/天叮/地叮), toggled directly by the user in the UI. Nothing about the hand's own shape determines this, unlike almost every other pattern in this table. | — | Excluded by 門清叮. Also applies to all 3 special hands, same caveat as 自摸/門清自摸 above. |
| 門清叮 (Riichi while 門前清) | 10 | Riichi declared (`riichi === "riichi"`) **and** 門前清 both hold. | 叮 | Same shape as 自摸/門清自摸 - upgrade of 叮, excludes it, but stacks with 門前清 itself. Also applies to all 3 special hands, where it always wins over plain 叮. |
...
- **門清自摸/門清叮 always win over plain 自摸/叮 for all 3 special hands** (十三么/十六不搭/嚦咕嚦咕),
  since those hands are always fully concealed by construction (no declared melds ever - each has
  its own "declaredMelds.length > 0 -> null" guard) - `isConcealedExceptKongs` is unconditionally
  true there, so declaring self-draw or 叮 on one of these hands always gets the upgraded tai (3/10)
  rather than the plain version (1/5), the same way an equivalent always-concealed ordinary hand
  would. This sits a little oddly next to 十三么's own deliberate choice to *exclude* the plain
  門前清 pattern itself as "trivially true, not meaningful to list" (see the Foundation section) -
  by that same reasoning the upgrade is also "trivially earned" here, not a meaningfully distinct
  achievement the way it is for an ordinary hand that could have been exposed. Kept as a genuine
  stack (not suppressed) per the user's explicit call, flagged here as worth revisiting rather than
