...
| 自摸 (Self-drawn win) | 1 | `selfDraw` is true. | — | Excluded by 門清自摸. Also applies to all 3 special hands (十三么/十六不搭/嚦咕嚦咕) - but 門清自摸 deliberately never fires there (see its own row), so this is always the one that applies when self-drawn. **UI-only wiring, not part of `selfDraw` itself**: the 自摸 button also lights up (and `GameContext.selfDraw` is forced true) whenever 花摸, 槓摸, 海底撈月/海底撈月(一筒), or 天胡 is declared - all mean the winning tile was drawn/dealt to the player themselves, which is inherently a self-draw. Tapping 自摸 off while held on this way cascades all of those back to their off state too, rather than leaving them declared with no visible way to un-toggle them. Conversely, 自摸 (in any of these forms) is mutually exclusive with 搶槓, 雙響/三響, and 地胡 below - all mean the win was claimed off another player, the opposite of a self-draw - so activating 自摸 clears all three, and activating any of them clears 自摸 (and, transitively, 花摸/槓摸/海底撈月/天胡 along with it). 人胡 is deliberately left unlinked either way - see its own row. |
...
| 食叮 | 5 | `eatRiichi` is true **and** `riichi` isn't `"none"`. | — | Independent of 一發 - both can be declared at once, each contributing their own flat 5 tai. Same UI show/reset behavior as 一發. Also applies to all 3 special hands. |
...
| 河底撈魚 | 5 | `lastTileWin === "river-bottom"` - a plain declared state (`GameContext.lastTileWin`, a 4-way cycle: none/河底撈魚/海底撈月/海底撈月一筒), toggled by the user in the UI, independent of everything else in this table. Won by claiming another player's very last discard of the game. | — | Mutually exclusive with 海底撈月/海底撈月(一筒) by construction. Also applies to all 3 special hands. **UI-only wiring**: mutually exclusive with 四子內/七子內/十子內 (see their own row) - no 自摸 wiring, unlike 海底撈月/海底撈月(一筒) below. |
