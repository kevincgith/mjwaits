...
| 食叮 | 5 | `eatRiichi` is true **and** `riichi` isn't `"none"`. | — | Independent of 一發 - both can be declared at once, each contributing their own flat 5 tai. Also applies to all 3 special hands. **UI-only wiring**: unlike 一發, 食叮 is its own always-visible button (not hidden until 叮 is declared) - it simply scores 0 while `riichi === "none"`, same as any other declared-but-inapplicable toggle. Joins the 搶槓/雙響/三響/地胡 "claimed win" group (see 自摸's own row): eating into the completed hand off a discard is a claimed win, so activating 食叮 forces 自摸 off (and 花摸/槓摸/河底撈魚/海底撈月/天胡 with it), and activating 自摸 forces 食叮 back off. |
