...
| 自摸 (Self-drawn win) | 1 | `selfDraw` is true. | — | Excluded by 門清自摸. Also applies to all 3 special hands (十三么/十六不搭/嚦咕嚦咕) - but 門清自摸 deliberately never fires there (see its own row), so this is always the one that applies when self-drawn. |
