...
| 雙響 | 5 | `multiWin === "double"` - a plain declared state (`GameContext.multiWin`, a 3-way cycle: none/雙響/三響), toggled by the user in the UI, independent of everything else in this table. Means multiple players won off the same discard. | — | Mutually exclusive with 三響 by construction. Also applies to all 3 special hands. |
| 三響 | 10 | Same shape as 雙響, `multiWin === "triple"`. | — | Mutually exclusive with 雙響 by construction. Also applies to all 3 special hands. |
...
| 搶槓 | `FIVE_POWER_TAI_TABLE[n]` | Same shape as 槓摸 (`GameContext.robKong`, same table), fully independent of it - robbing the kong (winning by claiming a tile another player just added to upgrade an existing pon into a kong). | — | Also applies to all 3 special hands. |
