...
| 花摸 | 2 each | `flowerDraw * 2` - `GameContext.flowerDraw` is a declared count, 0-8 (cycled by the UI, labeled 花摸x1 through 花摸x8), independent of everything else in this table. Means a bonus-tile replacement draw completed/advanced the hand, that many times. | — | Purely declared - the app has no concept of draw order, so unlike a real 花摸 ruling (which genuinely depends on exactly when each bonus tile was revealed relative to the winning tile) this just trusts the user's count. Also applies to all 3 special hands. |
| 槓摸 | `FIVE_POWER_TAI_TABLE[n]` | `GameContext.kongDraw`, a declared count 0-5 (cycled by the UI, labeled 槓摸x1 through 槓摸x5). Tai follows the literal table `[0, 5, 25, 125, 250, 250]` indexed by the count - **not** a clean `5^n` formula (5⁴ and 5⁵ both cap at 250, per the user's own worked numbers, not the `min(5^n, 240)` they also mentioned - the explicit list is treated as ground truth over the mismatched formula description). | — | Same "purely declared, trust the user" reasoning as 花摸 - a kong-replacement draw completed/advanced the hand, that many times. Also applies to all 3 special hands. |
