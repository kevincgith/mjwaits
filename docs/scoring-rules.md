...
| 自摸 (Self-drawn win) | 1 | `selfDraw` is true. | — | Excluded by 門清自摸. Also applies to all 3 special hands (十三么/十六不搭/嚦咕嚦咕) - but 門清自摸 deliberately never fires there (see its own row), so this is always the one that applies when self-drawn. **UI-only wiring, not part of `selfDraw` itself**: the 自摸 button also lights up (and `GameContext.selfDraw` is forced true) whenever 花摸 or 槓摸 is declared - both mean the winning tile was a replacement tile the player drew themselves, which is inherently a self-draw. Tapping 自摸 off while held on this way cascades 花摸/槓摸 back to 0 too, rather than leaving them declared with no visible way to un-toggle them. Conversely, 自摸 (in any of these forms) is mutually exclusive with 搶槓 and 雙響/三響 below - all three mean the win was claimed off another player, the opposite of a self-draw - so activating 自摸 clears both of those, and activating either of those clears 自摸 (and, transitively, 花摸/槓摸 along with it). |
...
| 四子內 (Won within 4 discards) | 60 | `earlyWin === "four"` - a plain declared state (`GameContext.earlyWin`, a 4-way cycle: none/四子內/七子內/十子內, independent of 叮), toggled by the user in the UI. Means the hand was won while the total discard count (excluding the tile that completed the hand) was still at or under the stated number. | — | Mutually exclusive with 七子內/十子內 by construction (`earlyWin` is a single state). Also applies to all 3 special hands. |
| 七子內 (Won within 7 discards) | 30 | Same shape as 四子內, `earlyWin === "seven"`. | — | Mutually exclusive with 四子內/十子內 by construction. Also applies to all 3 special hands. |
| 十子內 (Won within 10 discards) | 15 | Same shape as 四子內, `earlyWin === "ten"`. | — | Mutually exclusive with 四子內/七子內 by construction. Also applies to all 3 special hands. |
| 雙響 | 5 | `multiWin === "double"` - a plain declared state (`GameContext.multiWin`, a 3-way cycle: none/雙響/三響), toggled by the user in the UI, independent of everything else in this table. Means multiple players won off the same discard. | — | Mutually exclusive with 三響 by construction. Also applies to all 3 special hands. **UI-only wiring** (see 自摸's own row): activating 雙響/三響 forces 自摸 off (and 花摸/槓摸 back to 0 with it), and activating 自摸 forces 雙響/三響 back to none - a discard-claimed win and a self-draw can't both be true. |
...
| 天胡 | 160 | `heavenlyWin === "heaven"` - a plain declared state (`GameContext.heavenlyWin`, a 4-way cycle: none/天胡/地胡/人胡), toggled by the user in the UI, independent of everything else in this table. | — | Mutually exclusive with 地胡/人胡 by construction. Also applies to all 3 special hands. |
| 地胡 | 120 | Same shape as 天胡, `heavenlyWin === "earth"`. | — | Mutually exclusive with 天胡/人胡 by construction. Also applies to all 3 special hands. |
| 人胡 | 80 | Same shape as 天胡, `heavenlyWin === "man"`. | — | Mutually exclusive with 天胡/地胡 by construction. Also applies to all 3 special hands. |
| 河底撈魚 | 5 | `lastTileWin === "river-bottom"` - a plain declared state (`GameContext.lastTileWin`, a 4-way cycle: none/河底撈魚/海底撈月/海底撈月一筒), toggled by the user in the UI, independent of everything else in this table. Won by claiming another player's very last discard of the game. | — | Mutually exclusive with 海底撈月/海底撈月(一筒) by construction. Also applies to all 3 special hands. |
| 海底撈月 | 10 | Same shape as 河底撈魚, `lastTileWin === "sea-bottom"`. Won by self-drawing the very last tile in the wall. | — | Mutually exclusive with 河底撈魚/海底撈月(一筒) by construction. Also applies to all 3 special hands. |
| 海底撈月(一筒) | 20 | Same shape as 河底撈魚, `lastTileWin === "sea-bottom-one-tong"` - 海底撈月 specifically when that final self-drawn tile is 1 Tong (`1t`). | — | Mutually exclusive with 河底撈魚/海底撈月 by construction. Also applies to all 3 special hands. |
